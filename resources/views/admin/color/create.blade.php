...
@section('content')
    <div class="container form-container mb-5 mt-4" style="min-height: 286px;">
       ...
       @enderror
        <form id="createProductForm" action="{{ url('admin/color') }}" method="post" enctype="multipart/form-data" style="width: 100%;">
           @csrf
             ...
             @enderror
   		    ...
   		    <div class="form-group mb-3">
			    <label for="hexcode">Color Hexcode</label>
                 <input style="padding:0;" id="hexcode" type="color" name="hexcode" class="form-control @error('hexcode') is-invalid @enderror" value="{{ old('hexcode') }}" required>
              @error('hexcode')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
            ...
            <input class="btn btn-primary mt-3 btn-lg" type="submit" value="Create Color"/>
        </form>
    </div>
@endsection