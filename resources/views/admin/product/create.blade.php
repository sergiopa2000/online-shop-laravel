...
@section('content')
    <div class="container form-container mb-5 mt-4">
       ...
       @enderror
        <form id="createProductForm" action="{{ url('admin/product') }}" method="post" enctype="multipart/form-data" style="width: 100%;">
           @csrf
	       ...
	       <div class="form-group mb-3">
   		    ...
   		    <div class="form-group mb-3">
		        ...
		        <div class="tag-container">
    		    	...
    		    	<div style="width:fit-content;" class="tag-input-container flex-c-m stext-107 cl6 size-301 bor7 p-lr-15 hov-tag1 trans-04 m-r-5 m-b-5">
                        <input value="0" type="hidden" name="tags[{{$tag->id}}]"/>
                        {{ $tag->name }}
    				</div>
    				@endforeach
				</div>
   		    ...
   		    <div class="form-group mb-3">
		        ...
		        <div class="tag-container">
    		    	...
    		    	<div data-class="{{ $color->name }}-active" style="width:fit-content;" class="color-input-container {{ $color->name }} flex-c-m stext-107 cl6 size-301 bor7 p-lr-15 hov-tag1 trans-04 m-r-5 m-b-5">
                        <input value="0" type="hidden" name="colors[{{$color->id}}]"/>
                        {{ $color->name }}
    				</div>
    				@endforeach
				</div>
            ...
            <input class="btn btn-primary mt-3 btn-lg" type="submit" value="Create Product"/>
        </form>
    </div>
@endsection
